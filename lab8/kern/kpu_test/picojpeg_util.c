...
// On success, the kmalloc()'d image's width/height is written to *x and *y, and
...
// Not thread safe.
// If reduce is non-zero, the image will be more quickly decoded at
// approximately 1/8 resolution (the actual returned resolution will depend on
// the JPEG subsampling factor).
uint8 *pjpeg_load_from_file(uint8_t *buf, uint32_t buf_len, int *x, int *y,
                            int *comps, pjpeg_scan_type_t *pScan_type,
                            int reduce, uint8_t rgb565, uint8_t *pixels,
                            int *err)
{
  ...
  *err = 0;
  ...
  status = pjpeg_decode_init(&image_info, pjpeg_need_bytes_callback, &f_info,
  ...
  if (pixels == NULL)
  {
    pImage = (uint8 *)kmalloc(row_pitch * decoded_height);
  }
  else
  {
    if ((row_pitch * decoded_height) > (LCD_X_MAX * LCD_Y_MAX * 2))
    {
      cprintf("image: max supported size: %dx%x\r\n", LCD_Y_MAX, LCD_X_MAX);
      *err = -1;
      return NULL;
    }
    pImage = pixels;
  ...
  for (;;)
  {
    ...
    if (reduce)
    {
      ...
      pDst_row = pImage + mcu_y * col_blocks_per_mcu * row_pitch +
                 mcu_x * row_blocks_per_mcu * one_color_size;
          ...
          for (x = 0; x < row_blocks_per_mcu; x++)
          {
            if (rgb565)
            {
              unsigned short color =
                  RGB888ToRGB565(image_info.m_pMCUBufR[src_ofs] << 16 |
                                 image_info.m_pMCUBufG[src_ofs] << 8 |
    ...
    else
    {
      ...
      pDst_row = pImage + (mcu_y * image_info.m_MCUHeight) * row_pitch +
                 (mcu_x * image_info.m_MCUWidth * one_color_size);

      for (y = 0; y < image_info.m_MCUHeight; y += 8)
      {
        const int by_limit =
            min(8, image_info.m_height - (mcu_y * image_info.m_MCUHeight + y));
        ...
        for (x = 0; x < image_info.m_MCUWidth; x += 8)
        {
          ...
          const int bx_limit =
              min(8, image_info.m_width - (mcu_x * image_info.m_MCUWidth + x));
          ...
          else
          {
            ...
            for (by = 0; by < by_limit; by++)
            {
              ...
              for (bx = 0; bx < bx_limit; bx++)
              {
                ...
                {
                  unsigned short color =
                      RGB888ToRGB565(*pSrcR << 16 | *pSrcG << 8 | *pSrcB);
                  pDst[0] = color >> 8 & 0xff;
                  pDst[1] = color & 0xff;
                }
                else
...
static void get_pixel(int *pDst, const uint8 *pSrc, int luma_only, int
num_comps)
{
   int r, g, b;
   if (num_comps == 1)
...
static void image_compare(image_compare_results *pResults, int width, int
height, const uint8 *pComp_image, int comp_image_comps, const uint8
*pUncomp_image_data, int uncomp_comps, int luma_only)
{
   double hist[256];
   ...
   for (y = 0; y < height; y++)
   {
      for (x = 0; x < width; x++)
      {
         ...
         get_pixel(a, pComp_image + (y * width + x) * comp_image_comps,
luma_only, comp_image_comps); get_pixel(b, pUncomp_image_data + (y * width + x)
* uncomp_comps, luma_only, uncomp_comps);
...
void jpeg_display(uint16_t startx, uint16_t starty, jpeg_image_t *jpeg)
{
  ...
  lcd_set_area(startx, starty, startx + jpeg->width - 1,
               starty + jpeg->height - 1);

  for (uint32_t i = 0; i < (jpeg->width * jpeg->height * 2); i += 4)
  {
    t[0] = *(ptr + 1);
    t[1] = *(ptr);
    *(ptr) = SWAP_16(t[0]);
  ...
  cprintf("jpeg->width * jpeg->height / 2  : %d\n",
          jpeg->width * jpeg->height / 2);
  tft_write_word(p, (jpeg->width * jpeg->height / 2), 0);
  // return p;
}
void jpeg_to_ai(jpeg_image_t *jpeg)
{
  uint32_t *ptr = jpeg->img_data;
  uint32_t t[2];
  for (uint32_t i = 0; i < (jpeg->width * jpeg->height * 3); i += 2)
  {
    t[0] = *(ptr + 1);
    t[1] = *(ptr);
    *(ptr) = t[0];
...
jpeg_image_t *pico_jpeg_decode(uint8_t *buf, uint32_t buf_len)
{
  ...
  if (img_data == NULL)
  {
    cprintf("Failed loading source image! err:%s\r\n",
            (err == -1) ? "EINVAL" : (err == -2) ? "ENOMEM"
                                                 : "EPERM");
    return NULL;
  }

  jpeg_image_t *jpeg = (jpeg_image_t *)kmalloc(sizeof(jpeg_image_t));

...
int cnn_run_dma_input_done_push_layers(void *_task)
{
    ...
    cnn_run_dma_output(task->dma_ch, task->dst, last_layer->dma_parameter.data.dma_total_byte + 1, task);
...
void cnn_run_dma_input(uint32_t dma_ch, void *src, void *_task)
{
...
int cnn_run_dma_output(uint32_t dma_ch, void *dst, uint32_t length, void *_task)
{
    LOG("_start %s [cnn] start run\n", __func__);
...
int cnn_run(cnn_task_t *task, int dma_ch, void *src, void *dst)
{
    LOG("_start %s [cnn] start run\n", __func__);
    cnn_layer_argument_t *last_layer = &task->layers[task->length - 1];
    cnn_layer_argument_t *first_layer = &task->layers[0];
    ...
    cnn_run_dma_input(dma_ch, src, task);
